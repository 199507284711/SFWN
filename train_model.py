...
def plot_train_test_loss(loss_train, train, loss_test, test):
    ...
    plt.savefig('training loss and bacc.eps', ppi=600, format='eps')
    plt.show()
    # plt.close()

def plot_weight_Graph(weight):
    weight = model.sgcn.weight.detach().numpy()
    gene_A = pd.read_table('../...', index_col=0)
...
for it in range(1,11):
    ...
    # x1 = pd.read_table('../...', index_col=0)
    # x2 = pd.read_table('../...', index_col=0)
    # x3 = pd.read_table('../...', index_col=0)
    # gene_A = pd.read_table('../...', index_col=0)
    ...
    positive = np.asarray(pd.read_csv('./...'%it, sep='\t', header=None)).reshape(-1)
    gene_A = np.loadtxt('./...'%it)
    x = np.asarray(pd.read_csv('./...'%it, sep='\t'))
    y = np.asarray([0] * 100 + [1] * 100 + [2] * 100 + [3] * 100)
    ###

  
    # positive = np.loadtxt('./...' % it)
    # gene_A = np.loadtxt('./...' % it)
    ...
    for train_portion in train_portions:
        ...
        for re in range(5):
            ...
            # plot_weight(var_left, 'a. left')
            # plot_weight(var_right, 'b. right')
            # plot_weight(var_importance, 'c. importance')
            # plot_weight(var_left_mean, 'd. SFWN_left')
            # plot_weight(var_right_mean, 'e. SFWN_right')
            # plot_weight(var_importance_mean, 'f. SFWN_importance')
